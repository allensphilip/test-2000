...
func AnalyzeTranscripts(ctx context.Context, logger *zap.Logger, bucket, originalFilePath, transcribedFilePath string) (*AnalysisResult, error) {
	sugar := logger.Sugar()
	sugar.Info("Starting transcription analysis process")

	// Log high-level operation without sensitive details
	sugar.Debug("Processing reference file for transcription analysis")
	...
	if err != nil {
		sugar.Errorw("File download failed",
			"error", err)
		return nil, fmt.Errorf("failed to download original file: %w", err)
	}
	sugar.Debugw("Successfully downloaded reference file",
	...
	sugar.Debugw("Successfully downloaded hypothesis file",
		"size_bytes", len(hypothesis))

	// Compute metrics
	metrics, err := computeMetrics(reference, hypothesis)
	if err != nil {
		sugar.Errorw("Metrics computation failed", "error", err)
	...
	sugar.Infow("Transcription analysis completed successfully",
		"wer", metrics.WER,
		"cer", metrics.CER,
		"bleu", metrics.BLEU)
...
func AnalyzeSummary(ctx context.Context, logger *zap.Logger, bucket, originalFilePath, summaryFilePath string) (*AnalysisResult, error) {
	sugar := logger.Sugar()
	sugar.Info("Starting summary analysis process")

	// Log high-level operation without sensitive details
	sugar.Debug("Processing original file for summary analysis")
	...
	sugar.Debugw("Successfully downloaded original file",
		"size_bytes", len(original))

	// Log high-level operation without sensitive details
	sugar.Debug("Processing summary file for analysis")
	...
	sugar.Debugw("Successfully downloaded summary file",
	...
	sugar.Infow("Summary analysis completed successfully",
		"wer", metrics.WER,
		"cer", metrics.CER,
		"bleu", metrics.BLEU)
