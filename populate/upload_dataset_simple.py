#!/usr/bin/env python3
"""
Analytics Dataset Uploader (Without Metadata)

Uploads dataset files WITHOUT metadata support.
Works with current analytics deployment (before metadata trigger changes).

Dataset Structure:
- Transcription: job-XXX_transcribed.txt + job-XXX_corrected.txt
- Summary: sum-XXX_original.txt + sum-XXX_summary.txt
...
class SimpleUploader:
    ...
    def upload_transcription(self, job_id):
        ...
        try:
            ...
            # Trigger analysis (no body, old endpoint style)
    ...
    def upload_summary(self, job_id):
        """Upload summary dataset (no metadata)"""
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                self.log(f"  ✓ Triggered analysis (no metadata)", 'SUCCESS')
                return True
            else:
                self.log(f"  ✗ Trigger failed: {response.status_code} - {response.text}", 'ERROR')
                return False
    ...
    def upload_all(self):
        """Upload all datasets"""
        self.log(f"\n{'='*60}")
        self.log(f"Analytics Dataset Uploader (Simple - No Metadata)")
        ...
        if summary_jobs:
            self.log(f"\n{'─'*60}")
            self.log("SUMMARY DATASETS (NO METADATA)")
            self.log(f"{'─'*60}\n")
...
def main():
    print("""
╔════════════════════════════════════════════════════════════╗
║     Analytics Dataset Uploader (Simple - No Metadata)      ║
║  Works with current analytics deployment (pre-metadata)    ║
╚════════════════════════════════════════════════════════════╝
    """)
    ...
    print(f"\n⚠️  Note: Metadata files will be IGNORED (requires new deployment)")
    print()
    
    uploader = SimpleUploader(ANALYTICS_API_URL)
